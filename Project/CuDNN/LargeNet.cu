...
DEFINE_int32(iterations, 15000, "Number of iterations for training");
...
struct TrainingContext
{
    ...
    cudnnTensorDescriptor_t dataTensor, conv1Tensor, conv1BiasTensor, pool1Tensor, 
                             conv2Tensor, conv2BiasTensor, pool2Tensor, conv3Tensor, conv3BiasTensor, pool3Tensor, fc1Tensor, fc2Tensor;
    cudnnFilterDescriptor_t conv1filterDesc, conv2filterDesc, conv3filterDesc;
    cudnnConvolutionDescriptor_t conv1Desc, conv2Desc, conv3Desc;
    cudnnConvolutionFwdAlgo_t conv1algo, conv2algo, conv3algo;
    cudnnConvolutionBwdFilterAlgo_t conv1bwfalgo, conv2bwfalgo, conv3bwfalgo;
    cudnnConvolutionBwdDataAlgo_t conv2bwdalgo,conv3bwdalgo;
    ...
    TrainingContext(int gpuid, int batch_size,
                    ConvBiasLayer& conv1, MaxPoolLayer& pool1, ConvBiasLayer& conv2, MaxPoolLayer& pool2, ConvBiasLayer& conv3, MaxPoolLayer& pool3,
                    FullyConnectedLayer& fc1, FullyConnectedLayer& fc2) : ref_fc1(fc1), ref_fc2(fc2), m_gpuid(gpuid)
    {
        ...
        checkCUDNN(cudnnCreateTensorDescriptor(&conv3Tensor));
        checkCUDNN(cudnnCreateTensorDescriptor(&conv3BiasTensor));
        
        checkCUDNN(cudnnCreateTensorDescriptor(&pool2Tensor));
        checkCUDNN(cudnnCreateTensorDescriptor(&pool3Tensor));
        ...
        checkCUDNN(cudnnCreateFilterDescriptor(&conv3filterDesc));

        checkCUDNN(cudnnCreateConvolutionDescriptor(&conv1Desc));
        checkCUDNN(cudnnCreateConvolutionDescriptor(&conv2Desc));
        checkCUDNN(cudnnCreateConvolutionDescriptor(&conv3Desc));
        ...
        checkCUDNN(cudnnSetTensor4dDescriptor(conv3BiasTensor,
                                              CUDNN_TENSOR_NCHW,
                                              CUDNN_DATA_FLOAT,
	...
	checkCUDNN(cudnnSetTensor4dDescriptor(pool3Tensor,
	                                      CUDNN_TENSOR_NCHW,
	                                      CUDNN_DATA_FLOAT,
	                                      batch_size, conv3.out_channels,
	                                      conv3.out_height / pool3.stride,
	                                      conv3.out_width / pool3.stride));
        ...
        workspace = std::max(workspace, SetFwdConvolutionTensors(conv3, pool2Tensor, conv3Tensor, conv3filterDesc, conv3Desc, conv3algo));
        workspace = std::max(workspace, SetBwdConvolutionTensors(pool2Tensor, conv3Tensor, conv3filterDesc, conv3Desc, &conv3bwfalgo, &conv3bwdalgo));
    ...
    ~TrainingContext()
    {
        ...
        checkCUDNN(cudnnDestroyTensorDescriptor(conv3Tensor));
        checkCUDNN(cudnnDestroyTensorDescriptor(conv3BiasTensor));
        checkCUDNN(cudnnDestroyTensorDescriptor(pool3Tensor));
        ...
        checkCUDNN(cudnnDestroyFilterDescriptor(conv3filterDesc));
        
        checkCUDNN(cudnnDestroyConvolutionDescriptor(conv1Desc));
        checkCUDNN(cudnnDestroyConvolutionDescriptor(conv2Desc));
        checkCUDNN(cudnnDestroyConvolutionDescriptor(conv3Desc));
    ...
    void ForwardPropagation(float *data, float *conv1, float *pool1, float *conv2, float *pool2, float *conv3, float *pool3, float *fc1, float *fc1relu,
                            float *fc2, float *result,
                            float *pconv1, float *pconv1bias, 
                            float *pconv2, float *pconv2bias, 
                            float *pconv3, float *pconv3bias,
    ...
    {        
        ...
        //---------------------- ASSEL МОИ ДОБАВЛЕНИЯАААА ------------------------------------
        
        // Conv3 layer
        checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, pool2Tensor,
                                           pool2, conv3filterDesc, pconv3, conv3Desc, 
                                           conv3algo, workspace, m_workspaceSize, &beta,
                                           conv3Tensor, conv3));
                                           
        checkCUDNN(cudnnAddTensor(cudnnHandle, &alpha, conv3BiasTensor,
                                  pconv3bias, &alpha, conv3Tensor, conv3));

        // Pool3 layer
        checkCUDNN(cudnnPoolingForward(cudnnHandle, poolDesc, &alpha, conv3Tensor,
                                       conv3, &beta, pool3Tensor, pool3));
        ...
        // FC1 layer
        // Forward propagate neurons using weights (fc1 = pfc1'*pool3)
        checkCudaErrors(cublasSgemm(cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N,
                                    ref_fc1.outputs, m_batchSize, ref_fc1.inputs,
                                    &alpha,
                                    pfc1, ref_fc1.inputs,
                                    pool3, ref_fc1.inputs,
                                    &beta,
                                    fc1, ref_fc1.outputs));
    ...
    void Backpropagation(ConvBiasLayer& layer_conv1, MaxPoolLayer& layer_pool1, ConvBiasLayer& layer_conv2, MaxPoolLayer& layer_pool2, ConvBiasLayer& layer_conv3, MaxPoolLayer& layer_pool3,
                         float *data, float *labels, float *conv1, float *pool1, float *conv2, float *pool2, float *conv3, float *pool3, float *fc1, float *fc1relu,
                         ...
                         float *gconv3, float *gconv3bias, float *dconv3, float *dpool3,
    ...
    {    
        ...
        // FC1 layer
        // Compute derivative with respect to weights: gfc1 = (pool3 * dfc1relu')
        checkCudaErrors(cublasSgemm(cublasHandle, CUBLAS_OP_N, CUBLAS_OP_T, ref_fc1.inputs, ref_fc1.outputs, m_batchSize,
                                    &alpha, pool3, ref_fc1.inputs, dfc1relu, ref_fc1.outputs, &beta, gfc1, ref_fc1.inputs));
        ...
        checkCUDNN(cudnnPoolingBackward(cudnnHandle, poolDesc, &alpha, 
                                        pool3Tensor, pool3, pool3Tensor, dfc1,
                                        conv3Tensor, conv3, &beta, conv3Tensor, dpool3));
        
        // Conv3 layer
        checkCUDNN(cudnnConvolutionBackwardBias(cudnnHandle, &alpha, conv3Tensor,
                                                dpool3, &beta, conv3BiasTensor, gconv3bias));

        
        checkCUDNN(cudnnConvolutionBackwardFilter(cudnnHandle, &alpha, pool2Tensor,
                                                  pool2, conv3Tensor, dpool3, conv3Desc,
                                                  conv3bwfalgo, workspace, m_workspaceSize,
                                                  &beta, conv3filterDesc, gconv3));
    
        checkCUDNN(cudnnConvolutionBackwardData(cudnnHandle, &alpha, conv3filterDesc,
                                                pconv3, conv3Tensor, dpool3, conv3Desc, 
                                                conv3bwdalgo, workspace, m_workspaceSize,
                                                &beta, pool2Tensor, dconv3));
        ...
        // Pool2 layer
        checkCUDNN(cudnnPoolingBackward(cudnnHandle, poolDesc, &alpha, 
                                        pool2Tensor, pool2, pool2Tensor, dconv3,
    ...
    void UpdateWeights(float learning_rate,
                       ConvBiasLayer& conv1, ConvBiasLayer& conv2, ConvBiasLayer& conv3,
                       float *pconv1, float *pconv1bias,
                       float *pconv2, float *pconv2bias,
                       float *pconv3, float *pconv3bias,
                       float *pfc1, float *pfc1bias,
    ...
    {    
        ...
        // Conv3
        checkCudaErrors(cublasSaxpy(cublasHandle, static_cast<int>(conv3.pconv.size()),
                                    &alpha, gconv3, 1, pconv3, 1));
        checkCudaErrors(cublasSaxpy(cublasHandle, static_cast<int>(conv3.pbias.size()),
                                    &alpha, gconv3bias, 1, pconv3bias, 1));
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    ConvBiasLayer conv2(conv1.out_channels, 50, 1, conv1.out_width / pool1.stride, conv1.out_height / pool1.stride);
    MaxPoolLayer pool2(2, 2);
    ConvBiasLayer conv3(conv2.out_channels, 70, 1, conv2.out_width / pool2.stride, conv2.out_height / pool2.stride);

    MaxPoolLayer pool3(2, 2);
    printf("hello");
    
    FullyConnectedLayer fc1((conv3.out_channels*conv3.out_width*conv3.out_height) / (pool3.stride * pool3.stride), 
                            500);
    FullyConnectedLayer fc2(fc1.outputs, 10);

    // Initialize CUDNN/CUBLAS training context
    TrainingContext context(FLAGS_gpu, FLAGS_batch_size, conv1, pool1, conv2, pool2, conv3, pool3, fc1, fc2);
    ...
    if (!bRet || !FLAGS_pretrained)
    {
        ...
        float wconv3 = sqrt(3.0f / (conv3.kernel_size * conv3.kernel_size * conv3.in_channels));
        std::uniform_real_distribution<> dconv3(-wconv3, wconv3);
        ...
        for (auto&& iter : conv3.pconv)
            iter = static_cast<float>(dconv3(gen));
        for (auto&& iter : conv3.pbias)
            iter = static_cast<float>(dconv3(gen));
    ...
    float *d_data, *d_labels, *d_conv1, *d_pool1, *d_conv2, *d_pool2, *d_conv3, *d_pool3, *d_fc1, *d_fc1relu, *d_fc2, *d_fc2smax;
    ...
    checkCudaErrors(cudaMalloc(&d_conv3,   sizeof(float) * context.m_batchSize * conv3.out_channels * conv3.out_height                  * conv3.out_width));
    checkCudaErrors(cudaMalloc(&d_pool3,   sizeof(float) * context.m_batchSize * conv3.out_channels * (conv3.out_height / pool3.stride) * (conv3.out_width / pool3.stride)));
    ...
    // Network parameters
    float *d_pconv1, *d_pconv1bias, *d_pconv2, *d_pconv2bias, *d_pconv3, *d_pconv3bias;
    ...
    checkCudaErrors(cudaMalloc(&d_pconv3,     sizeof(float) * conv3.pconv.size()));
    checkCudaErrors(cudaMalloc(&d_pconv3bias, sizeof(float) * conv3.pbias.size()));
    ...
    float *d_gconv1, *d_gconv1bias, *d_gconv2, *d_gconv2bias, *d_gconv3, *d_gconv3bias;
    ...
    checkCudaErrors(cudaMalloc(&d_gconv3,     sizeof(float) * conv3.pconv.size()));
    checkCudaErrors(cudaMalloc(&d_gconv3bias, sizeof(float) * conv3.pbias.size()));
    ...
    float *d_dpool1, *d_dpool2, *d_dconv2, *d_dpool3, *d_dconv3, *d_dfc1, *d_dfc1relu, *d_dfc2, *d_dfc2smax, *d_dlossdata;
    //                         Buffer     | Element       | N                   | C                  | H                                 | W
    //-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    checkCudaErrors(cudaMalloc(&d_dpool3,   sizeof(float) * context.m_batchSize * conv3.out_channels * conv3.out_height                  * conv3.out_width));
    checkCudaErrors(cudaMalloc(&d_dconv3,   sizeof(float) * context.m_batchSize * conv2.out_channels * (conv2.out_height / pool1.stride) * (conv2.out_width / pool2.stride)));
    ...
    checkCudaErrors(cudaMemcpyAsync(d_pconv3, &conv3.pconv[0],     sizeof(float) * conv3.pconv.size(),  cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpyAsync(d_pconv3bias, &conv3.pbias[0], sizeof(float) * conv3.pbias.size(),  cudaMemcpyHostToDevice));
    ...
    for (int iter = 0; iter < FLAGS_iterations; ++iter)
    {
        ...
        // Forward propagation
        context.ForwardPropagation(d_data, d_conv1, d_pool1, d_conv2, d_pool2,d_conv3, d_pool3, d_fc1, d_fc1relu, d_fc2, d_fc2smax, 
                                   d_pconv1, d_pconv1bias, d_pconv2, d_pconv2bias, d_pconv3, d_pconv3bias, d_pfc1, d_pfc1bias, d_pfc2, d_pfc2bias,
                                   d_cudnn_workspace, d_onevec);

        // Backward propagation
        context.Backpropagation(conv1, pool1, conv2, pool2,  conv3, pool3,
                                d_data, d_labels, d_conv1, d_pool1, d_conv2, d_pool2, d_conv3, d_pool3, d_fc1, d_fc1relu, d_fc2, d_fc2smax, d_dlossdata,
                                d_pconv1, d_pconv1bias, d_pconv2, d_pconv2bias, d_pconv3, d_pconv3bias, d_pfc1, d_pfc1bias, d_pfc2, d_pfc2bias,
                                d_gconv1, d_gconv1bias, d_dpool1, d_gconv2, d_gconv2bias, d_dconv2, d_dpool2, d_gconv3, d_gconv3bias, d_dconv3, d_dpool3, d_gfc1, d_gfc1bias, 
        ...
        // Update weights
        context.UpdateWeights(learningRate, conv1, conv2, conv3,
                              d_pconv1, d_pconv1bias, d_pconv2, d_pconv2bias, d_pconv3, d_pconv3bias, d_pfc1, d_pfc1bias, d_pfc2, d_pfc2bias,
                              d_gconv1, d_gconv1bias, d_gconv2, d_gconv2bias, d_gconv3, d_gconv3bias, d_gfc1, d_gfc1bias, d_gfc2, d_gfc2bias);
    }
    checkCudaErrors(cudaDeviceSynchronize());
    ...
    if (FLAGS_save_data)
    {
        ...
        checkCudaErrors(cudaMemcpy(&conv3.pconv[0], d_pconv3, sizeof(float) * conv3.pconv.size(), cudaMemcpyDeviceToHost));
        checkCudaErrors(cudaMemcpy(&conv3.pbias[0], d_pconv3bias, sizeof(float) * conv3.pbias.size(), cudaMemcpyDeviceToHost));
    ...
    if (classifications > 0)
    {
        ...
        TrainingContext test_context(FLAGS_gpu, 1, conv1, pool1, conv2, pool2, conv3, pool3, fc1, fc2);
        ...
        for (int i = 0; i < classifications; ++i)
        {
            ...
            test_context.ForwardPropagation(d_data, d_conv1, d_pool1, d_conv2, d_pool2, d_conv3, d_pool3, d_fc1, d_fc1relu, d_fc2, d_fc2smax,
                                            d_pconv1, d_pconv1bias, d_pconv2, d_pconv2bias, d_pconv3, d_pconv3bias, d_pfc1, d_pfc1bias,
    ...
    checkCudaErrors(cudaFree(d_conv3));
    checkCudaErrors(cudaFree(d_pool3));
    ...
    checkCudaErrors(cudaFree(d_pconv3));
    checkCudaErrors(cudaFree(d_pconv3bias));
    ...
    checkCudaErrors(cudaFree(d_gconv3));
    checkCudaErrors(cudaFree(d_gconv3bias));
    ...
    checkCudaErrors(cudaFree(d_dconv3));
    checkCudaErrors(cudaFree(d_dpool3)); 
